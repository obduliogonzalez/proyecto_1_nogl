...
# Supongamos que tienes tus datos en un data frame llamado "datos" y has transformado a "transacciones" previamente

# Verifica los nombres de las columnas en el conjunto de datos original
colnames(datos)

# Muestra las transacciones y sus items únicos
inspect(transacciones[1:5]) # Ver las primeras 5 transacciones
itemLabels(transacciones)    # Ver los nombres de los items en las transacciones

# Convierte todas las columnas a factores (si aún no están en formato de factor)
datos[] <- lapply(datos, as.factor)

# Convierte a transacciones
transacciones <- as(datos, "transactions")


# Aplica Apriori con el valor correcto para `Niv_escolaridad_inf`
reglas_edad_escolaridad <- apriori(transacciones, 
                                   parameter = list(supp = 0.01, conf = 0.6, minlen = 2), 
                                   appearance = list(lhs = c("Edad_inf=20-29"), rhs = "Niv_escolaridad_inf=Secundaria"),
                                   control = list(verbose=FALSE))

# Inspecciona y visualiza las reglas encontradas
inspect(head(sort(reglas_edad_escolaridad, by = "lift"), 10))
plot(reglas_edad_escolaridad, method="graph", measure="lift", shading="confidence")
...
# Verifica los valores únicos en las columnas PEI3 y PEI4
unique(columnas_necesarias$PEI3)
unique(columnas_necesarias$PEI4)

reglas <- apriori(transacciones, 
                  parameter = list(supp = 0.01, conf = 0.5, minlen = 2),
                  appearance = list(lhs = c("PEI3=20"), rhs = "PEI4=1"),
                  control = list(verbose=FALSE))

# Inspecciona las reglas generadas
inspect(head(sort(reglas, by = "lift"), 10))

# Visualización opcional de las reglas
plot(reglas, method="graph", measure="lift", shading="confidence")
...
# Convierte las columnas a tipo factor para que arules pueda manejarlas
...
datos_geo <- datos[, c("DEPARTAMENTO", "MUNICIPIO")]
datos_geo[] <- lapply(datos_geo, as.factor)  # Convertir a factores

# Convierte a transacciones
transacciones_geo <- as(datos_geo, "transactions")

# Revisa los items en las transacciones
itemLabels(transacciones_geo)


reglas_geo <- apriori(transacciones_geo, 
                      parameter = list(supp = 0.005, conf = 0.5, minlen = 2),
                      appearance = list(lhs = "DEPARTAMENTO=X", rhs = "MUNICIPIO=Y"),
                      control = list(verbose = FALSE))

inspect(head(sort(reglas_geo, by = "lift"), 10))
